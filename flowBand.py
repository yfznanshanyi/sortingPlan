...
class FLOWBAND(object):
    ...
    def set_no_NC_loads(self):
        self.loads_no_NC = sum([flow.loads_no_NC for flow in self.flow_list])
        self.mean_loads_no_NC = self.loads_no_NC / len(self.flow_list)
        return self.loads_no_NC

    def set_NC_loads(self):
        self.loads_NC = sum([flow.loads_NC for flow in self.flow_list])
        self.mean_loads_NC = self.loads_NC / len(self.flow_list)
    ...
    # NC functions
    def NC_split_big_flow(self, sorting_sation_weight, sorting_sation_num_ub):
        if len(self.flow_list) == 1:
            split_num = self.loads_no_NC / sorting_sation_weight
            # print('loads: ',self.loads,' split_num: ',split_num)
            if split_num < CONSTDATA.sorting_sation_travel_level1_split_rate_lb:
                return [FLOWBAND(self.flow_list)]
            elif split_num > sorting_sation_num_ub:
    ...
    def NC_set_combined_flowBands(self, sorting_sation_weight, sorting_sation_travel_level1_combine_rate_ub):
        # flow
        temp_flow_list = copy.copy(self.flow_list)
        temp_flow_list.sort()
        combined_flowBands = []
        ...
        for flow in temp_flow_list:
            if len(temp_flowBand) == 0:
                temp_flowBand.append(copy.copy(flow))
            elif sum([flow.loads_no_NC for flow in temp_flowBand]) < sorting_sation_weight:
                weight_ub = sorting_sation_travel_level1_combine_rate_ub * sorting_sation_weight
                if sum([flow.loads_no_NC for flow in temp_flowBand]) + flow.loads_no_NC < weight_ub:
                    temp_flowBand.append(copy.copy(flow))
                else:
                    combined_flowBands.append(FLOWBAND(temp_flowBand))
                    temp_flowBand = []
