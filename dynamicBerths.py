...
class DYNAMICBERTHS(object):
    ...
    def set_day_sort_plan(self, duration=0):
        ...
        for index, d in enumerate(self.date_list):
            ...
            algs = ALGS(model, self.encoding, encoding_record)
            # algs.model.show_fit()
    ...
    def set_no_NC_figure(self):
        ...
        after_main = self.comprea_info_fit["after main_line"]
        after_ss_2_sa = self.comprea_info_fit["after sorting_sation_2_storage_area"]
        after_sa_2_lb = self.comprea_info_fit["after storage_area_2_loading_berth"]
